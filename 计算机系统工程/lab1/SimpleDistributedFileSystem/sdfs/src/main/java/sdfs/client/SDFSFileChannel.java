...
public class SDFSFileChannel implements SeekableByteChannel, Flushable {
    ...
    private DataNode dataNode = DataNode.getInstance();
...
//        初始化一些变量
        ...
        int blockIndex = (int) (position / dataNode.BLOCK_SIZE);//本次读取的第一个块的下标
...
//            判断当前获取块是否与需要读取的块相同，相同则进入读取
            if (blockIndex == index) {
                int offset = (int) (position % dataNode.BLOCK_SIZE);//当前块的偏移量
                int blockRemainSize = dataNode.BLOCK_SIZE - offset;//当前块剩余可读字节
                if (blockRemainSize < leftToRead) {
//                    当前剩余可读字节小于剩余需要读取的字节时，读取当前块剩余全部字节
                    byte[] bytes = dataNode.read(uuid, locatedBlock.getBlockNumber(), offset, blockRemainSize);
                    dst.put(bytes);
...
//                    当剩余可读字节大于需要读取的字节时，读取字节数为需要读取的字节
                    byte[] bytes = dataNode.read(uuid, locatedBlock.getBlockNumber(), offset, leftToRead);
                    dst.put(bytes);
                    position += leftToRead;
//                    由于需要读取字节读取完毕，直接跳出循环
                    break;
...
//        判断写入完成之后文件需要的block数量，并移除或添加block
        int needBlockNumber = fileSize % dataNode.BLOCK_SIZE == 0 ? fileSize / dataNode.BLOCK_SIZE : fileSize / dataNode.BLOCK_SIZE + 1;
        if (needBlockNumber > fileNode.getNumBlocks()) {
            nameNode.addBlocks(uuid, needBlockNumber - fileNode.getNumBlocks());
        } else if (needBlockNumber < fileNode.getNumBlocks()) {
            nameNode.removeLastBlocks(uuid, fileNode.getNumBlocks() - needBlockNumber);
        }
        Iterator<BlockInfo> iteratorBlockInfo = fileNode.iterator();
        int blockIndex = (int) (position / dataNode.BLOCK_SIZE);//本次写入第一次在的块下标
        int index = 0;
        while (iteratorBlockInfo.hasNext()) {
//            获取当前块
            Iterator<LocatedBlock> iteratorLocatedBlock = iteratorBlockInfo.next().iterator();
...
//            判断当前块是否是需要写入的块
            if (blockIndex == index) {
                int offset = (int) (position % dataNode.BLOCK_SIZE);//当前块写入位置偏移
                int blockRemainSize = dataNode.BLOCK_SIZE - offset;//当前块剩余可写入的大小
                if (blockRemainSize < leftToWrite) {
//                    当前块剩余可写入大小 小于 剩余需要写入的字节数时，将当前块写满
                    byte[] bytes = new byte[blockRemainSize];
                    src.get(bytes);
                    dataNode.write(uuid, locatedBlock.getBlockNumber(), offset, bytes);
//                    更新位置的变量
                    position += blockRemainSize;
...
//        当文件大小大于截断大小时截断，反之不必要处理
        if (fileNode.getFileSize() > size) {
            setFileSize(size);
            int needBlockNumber = size % dataNode.BLOCK_SIZE == 0 ? (int) (size / dataNode.BLOCK_SIZE) : (int) (size / dataNode.BLOCK_SIZE + 1);
            nameNode.removeLastBlocks(uuid, fileNode.getNumBlocks() - needBlockNumber);
        }
//        更新位置
        if (position > size) {
...
//            关闭时判断类型，选择合适的关闭
            if (isReadOnly) {
                ...
                nameNode.closeReadwriteFile(uuid, (int) position);
            }
