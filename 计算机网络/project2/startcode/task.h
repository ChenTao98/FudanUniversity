...
typedef struct up_conn_s {
    int up_id;//当前上传连接的id，每一个连接加1
    int ssthresh;//当前上传连接的慢启动阈值
    int is_slow_start;//记录是否是慢启动状态
    int receive_ack_during_avoid;//记录拥塞避免期间收到的ack数量
    pthread_t pid;//管理这个连接的time out的线程pid
    int expect_ack;//下一个期待的ack
    int last_sent;//上一个发送的包的序列号
    int available;//可以发送的包的最大序列号
    int duplicate;//记录收到重复ack数量
    int cwnd;//接收方窗口大小
    data_packet_t **pkts;//需要上传的数据包数组
    struct timeval timer;//记录时间
